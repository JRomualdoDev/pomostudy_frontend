...
import { ChangeDetectionStrategy, Component, computed, effect, input, output, signal, TemplateRef, ViewEncapsulation, Directive, Input, ViewContainerRef } from '@angular/core';
...
export class ZardStringTemplateOutletDirective {
  @Input('zStringTemplateOutlet') set template(value: TemplateRef<any> | null) {
    this.vcr.clear();
    if (value) {
      this.vcr.createEmbeddedView(value);
    }
  }
